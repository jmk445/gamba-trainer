...
async function transferFile$1(fileContents) {
  ...
  console.log("blefile$1 transfer enter succeeded");
...
export let isConnected$2 = false;
let hasModel = false;

export function isConnected$3() {
  // console.log("isconnected3");
...
function handleTransferPermissionChange(event) {
  ...
  if (values[0] == 1) {
    if (FromIDB == 1) {
      transferModelFromIDB();
      //handleClickTransfer();
    } else {
      transferModelFromLocal();
    }
  }
}
...
function onDisconnected(event) {
  //이거도 쓰임 isconnected$2 를 초기화 시켜주는 게 쓰이는 거라고 보면 됨.
  eventHandler$1.dispatchEvent("disconnect");
  alert("bluetooth disconnected! pls connect again");
  isConnected$2 = false;
}

//이거 쓰임 connect 해주는 함수가 불필요한게 너무 많은 것으로 추정, blemanagerapi.connect 에서만 호출됨.
...
const bleManagerApi = {
  async connect() {
    ...
    console.log('characteristic getting done!');
    ...
    //이거는 언제 실행되는지 모르겠음. 처음에 connect 할때는 안쓰임.
    isConnected$2 = true;
  },
  ...
  async transferFile(fileBuffer, fileType) {
    ...
    console.log("blemanager transfer enter succeeded");
...
function loadFile$1() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function (event) {
      ...
      console.log("transaction open succeeded");
...
async function connect() {
  ...
  _isConnected = true;
  console.log("this is _isconnected");
...
async function transferModelFromLocal(
  // numClasses,
  // threshold,
  // numSamples,
  // captureDelay,
  ...
  try {
    ...
    const modelBuffer = await loadFile();

    console.log("load file succeeded");
    // 연결이 안됐다면 연결 먼저 진행
    if (!isConnected$2) {
      await bleManagerApi.connect();
    }
...
async function transferModelFromIDB(
  // numClasses,
  // threshold,
  // numSamples,
  // captureDelay,
...
var api = /*#__PURE__*/Object.freeze({
  ...
  transferModelFromIDB: transferModelFromIDB,
  transferModelFromLocal: transferModelFromLocal,
...
//experimentConfig에서 onconnect처리를 해주기 위한 코드
...
export async function handleSendModel(flag) {
