...
async function handleHasModelChange(event) {
  ...
  if (value == 1) {
    // type signal 전송(7: application speech)
    await typeSignalTxChar.writeValue(Uint8Array.of(7));
  }
  else if (value == 0) {
...
const bleManagerApi = {
  ...
  async recordingSpeech() {
    console.log("test");
    const tempArray = Uint8Array.of(1);
    await recordingSpeechTxChar.writeValue(tempArray);
  },
...
function handleInference(data) {
  if (experimentConfig.onInference) {
    ...
    console.log("didn't got gesture" + data);
  } else {
    console.log(
...
export { handleClickConnect, getInferenceResult, handleSendModel, handleCheckModel, handleFileInput, addEventListener, getIsConnectedSpeech };
function getIsConnectedSpeech() {
  return isConnected;
}
let index = null;
...
async function handleClickConnect() {
  ...
  // }
  // else if(integerInput.value > 5) {
  //   integerInput.value = 5;
  // } 
  experimentConfigSetup();
...
async function handleCheckModel() {
  ...
  const sensorTypeArray = Uint8Array.of(1);
  await sensorTypeTxChar.writeValue(sensorTypeArray);

  const numClassesArray = Uint8Array.of(3);
  await numClassesTxChar.writeValue(numClassesArray);

  // TYPE_CHECK_MODEL_SPEECH
  await typeSignalTxChar.writeValue(Uint8Array.of(10));
}
...
async function handleSendModel() {
  ...
  const namesArray = "TEST1, test2, Test3"
  ...
  await typeSignalTxChar.writeValue(Uint8Array.of(4));
}
