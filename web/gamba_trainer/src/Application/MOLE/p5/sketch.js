...
export default function (p5) {
  ...
  p5.draw = function () {
    if (timeCount === 0 || timeCount === 100) {
      // console.log("start, posindex : ", posIndex);
      isScore = 0;
      p5.background(194, 242, 87);
  ...
  handleGesture = function (index) {
    if (index === "left" || index === "right" || index === "up" || index === "down") {
      attack = index;
    }
  ...
  pos = function () {
    posIndex = Math.floor(p5.random(0, 4));
    while (posIndex === posIndex_before) {
      posIndex = Math.floor(p5.random(0, 4));
    }
    posIndex_before = posIndex;
    return posIndex;
  };
  stop = function () {
    p5.noLoop();
...
export function triggerGesture(index) {
  console.log("triggerGesture" + index);
  if (!_p5) return;
  handleGesture(index);
}
