...
async function handleHasModelChange(event) {
  ...
  if (value == 1) {
    // type signal 전송(7: application vision)
    await typeSignalTxChar.writeValue(Uint8Array.of(8));
  }
  else if (value == 0) {
...
export { handleClickConnect, getInferenceResult, handleSendModel, handleCheckModel, handleFileInput, addEventListener, getIsConnectedVision };
function getIsConnectedVision() {
  return isConnected;
}
let index = null;
...
async function handleCheckModel() {
  ...
  const sensorTypeArray = Uint8Array.of(2);
  await sensorTypeTxChar.writeValue(sensorTypeArray);

  const numClassesArray = Uint8Array.of(2);
  await numClassesTxChar.writeValue(numClassesArray);

  // TYPE_CHECK_MODEL_VISION
  await typeSignalTxChar.writeValue(Uint8Array.of(11));
}
...
async function handleSendModel() {
  ...
  //upload vision: 5
  await typeSignalTxChar.writeValue(Uint8Array.of(5));
}
